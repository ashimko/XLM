...
#
N_MONO=8090  # number of monolingual sentences for each language
...
if [ "$SRC" != "nl" ]; then echo "unknown source language"; exit; fi
if [ "$TGT" != "ul" ]; then echo "unknown target language"; exit; fi
...
if [ "$SRC" == "nl" -a "$TGT" == "ul" ]; then
  PARA_SRC_VALID=$PARA_PATH/dev/valid.nl
  PARA_TGT_VALID=$PARA_PATH/dev/valid.ul
  PARA_SRC_TEST=$PARA_PATH/dev/test.nl
  PARA_TGT_TEST=$PARA_PATH/dev/test.ul
fi
...
echo "BPE codes applied to $TGT in: c$TGT_TRAIN_BPE"
